...
cd /d C:\Users\Administrator\Desktop\FE-Happy-Milk-Admin
...
IF EXIST "C:\inetpub\wwwroot\fe-admin-happy-milk" (
    echo Deleting existing files in fe-admin-happy-milk...
    CALL rmdir /s /q "C:\inetpub\wwwroot\fe-admin-happy-milk"
)

REM Tạo lại thư mục đích
CALL mkdir "C:\inetpub\wwwroot\fe-admin-happy-milk"

REM Sao chép các file đã build vào thư mục đích
echo Copying built files to fe-trade...
CALL xcopy "dist\fuse\browser\*" "C:\inetpub\wwwroot\fe-admin-happy-milk" /s /e /y
